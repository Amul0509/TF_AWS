...
provider "aws" {
  region = "us-east-1"
  access_key = "AKIAVEM4Z5LTTPEDXI5B"
  secret_key = "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"
}
...
resource "aws_s3_bucket_acl" "jenkins_artifacts_bucket" {
  bucket = "jenkins-s3bucket-amul2023" # Set a unique bucket name
...
resource "null_resource" "name" {
  ...
  connection {
    type        = "ssh"
    user        = "ec2-user"
    private_key = file("~/Downloads/IAMKeypair.pem")
